...
export default function EditRecipe() {
    ...
    useEffect(() => {
        ...
        const fetchRecipe = async () => {
            try {
                // In a real app: const res = await fetch(`/api/recipes/${id}`);
                // For now, we mock or try to fetch.
                ...
                if (res.ok) {
                    ...
                    // Mock data fallback for demo if API fails
                    setFormData({
                        title: 'Receita Exemplo (Editando)',
                        description: 'Descrição recuperada do banco de dados...',
                        category: 'Sobremesas',
                        time: '45 min',
                        difficulty: 'Fácil',
                        ingredients: ['Trigo', 'Ovos', 'Leite'],
                        status: 'published',
                        image: 'https://images.unsplash.com/photo-1563729768-7491b31bf71f?auto=format&fit=crop&w=1000&q=80'
                    });
                }
            } catch (error) {
                console.error(error);
    ...
    if (isFetching) {
        return <div className="p-8 text-white text-center">Carregando dados da receita...</div>;
    }
            ...
            <div className="flex items-center justify-between">
                <div>
                    <div className="flex items-center gap-2 text-stone-500 text-sm mb-1">
                        ...
                        <span>›</span>
                        <span className="text-stone-300">Editar Receita</span>
                    </div>
                    <h1 className="text-3xl font-bold text-white">Editar: {formData.title}</h1>
                </div>
                    ...
                    <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-2xl p-6">
                        ...
                        <div className="relative">
                            ...
                            <label
                                ...
                                className={`border-2 border-dashed border-stone-700 rounded-xl p-8 flex flex-col items-center justify-center gap-3 text-center hover:bg-[#252525] transition-all cursor-pointer group overflow-hidden relative ${formData.image ? 'border-none p-0 h-64' : ''
                                    }`}
                            >
                                {formData.image ? (
                                    <>
